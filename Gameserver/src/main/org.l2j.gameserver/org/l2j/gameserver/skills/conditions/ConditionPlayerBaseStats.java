...
public class ConditionPlayerBaseStats extends Condition
{
	...
	public boolean testImpl(Env env)
	{
		...
		{
			case Int:
				return player.getIntelligence() >= _value;
			case Str:
				return player.getStrength() >= _value;
			case Con:
				return player.getConstitution() >= _value;
			case Dex:
				return player.getDexterity() >= _value;
			case Men:
				return player.getMentality() >= _value;
			case Wit:
				return player.getWisdom() >= _value;
		}
		return false;
	}
}
