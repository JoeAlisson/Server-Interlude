...
public class L2DropCategory {
    ...
    public void addDropData(L2DropData drop) {
        if (drop.isQuestDrop()) {
            ...
            // for drop selection inside a category: max 100 % chance for getting an item, scaling allTemplates values to that.
    ...
    /**
     * ONE of the drops in this category is to be dropped now. to see which one will be dropped, weight allTemplates items' chances such that their sum of chances equals MAX_CHANCE. since the individual drops have their base chance, we also ought to use the base category chance for the weight. So weight =
     ...
     * %, meaning there are no big changes for low rate servers and no changes at allTemplates for 1x servers.
     *
