...
public class Duel {
    ...
    /**
     * Stops allTemplates players from attacking. Used for duel timeout / interrupt.
     */
    ...
    public void startDuel() {
        ...
        if (_partyDuel) {
            // set isInDuel() state
            // cancel allTemplates active trades, just in case? xD
            for (L2PcInstance temp : _playerA.getParty().getPartyMembers()) {
    ...
    public void restorePlayerConditions(boolean abnormalDuelEnd) {
        // update isInDuel() state for allTemplates players
        if (_partyDuel) {
            for (L2PcInstance temp : _playerA.getParty().getPartyMembers()) {
                temp.setIsInDuel(0);
                temp.setTeam(0);
    ...
    /**
     * teleport allTemplates players to the given coordinates
     *
     * @param x
     * @param y
     * @param z
    ...
    /**
     * Playback the bow animation for allTemplates loosers
     */
