...
/**
 * This class handles following admin commands: - kill = kills target L2Character - kill_monster = kills target non-reader - kill <radius> = If radius is specified, then ALL players only in that radius will be killed. - kill_monster <radius> = If radius is specified, then ALL non-players only in
				...
				if (plyr != null)
				{
					if (st.hasMoreTokens())
					{
						try
						{
							...
							activeChar.sendMessage("Killed allTemplates characters within a " + radius + " unit radius.");
							return true;
						}
