 ...
 */
package org.l2j.gameserver.status;
...
import org.l2j.gameserver.instancemanager.Manager;
...
import org.l2j.gameserver.util.DynamicExtension;
...
{
	// private static final Logger logger = LoggerFactory.getLogger(AdminTeleport.class.getName());
	...
	public void run()
	{
		...
		try
		{
			while ((_usrCommand.compareTo("quit") != 0) && (_usrCommand.compareTo("exit") != 0))
			{
				...
				{
					_print.println("The following is a list of allTemplates available commands: ");
				...
				{
					_print.println("The following is a list of allTemplates available debug commands: ");
				...
				/*
				 * else if (_usrCommand.startsWith("unblock")) { try { _usrCommand = _usrCommand.substring(8); if (LoginServer.getInstance().unblockIp(_usrCommand)) { logger.warn("IP removed via TELNET by host: " + _csocket.getInetAddress().getRemoteAddress()); _print.println("The IP " + _usrCommand
