...
import org.l2j.gameserver.model.L2Multisell.MultiSellEntry;
import org.l2j.gameserver.model.L2Multisell.MultiSellIngredient;
import org.l2j.gameserver.model.L2Multisell.MultiSellListContainer;
    ...
    private void doExchange(L2PcInstance player, MultiSellEntry templateEntry, boolean applyTaxes, boolean maintainEnchantment, int enchantment) {
        ...
        for (MultiSellIngredient e : _ingredientsList) {
            ...
            if (e.getItemId() != 65336) {
                // if this is not a list that maintains enchantment, check the count of allTemplates items that have the given id.
            ...
            if (e.getItemId() != 65336) {
                ...
                if (Config.ALT_BLACKSMITH_USE_RECIPES || !e.getMantainIngredient()) {
                    ...
                    if (itemToTake.isStackable()) {
                        ...
                        {
                            /*
                             * NOTE: There are 2 ways to achieve the above goal. 1) Get allTemplates items that have the correct itemId, loop through them until the lowest enchantment level is found. Repeat allTemplates this for the next item until proper count of items is reached. 2) Get allTemplates items that have the
                             ...
                             * b (just grab the m first items) or in linear time for case a (find the beginning of items with correct enchantment, index x, and take allTemplates items from x to x+m). Basically, whenever m > log(n) we have: m*(2n-m+1)/2 = (2nm-m*m+m)/2 > (2nlogn-logn*logn+logn)/2 = nlog(n) -
