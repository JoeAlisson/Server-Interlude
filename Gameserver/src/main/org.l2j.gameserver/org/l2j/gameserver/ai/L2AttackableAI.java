...
public class L2AttackableAI<T extends L2Attackable.AIAccessor> extends MovableAI<T> implements Runnable
{
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Update every 1s the _globalAggro counter to come close to 0</li> <li>If the actor is Aggressive and can attack, add allTemplates autoAttackable L2Character in its Aggro Range to its _aggroList, chose a target and order to attack it</li> <li>If the actor is a L2GuardInstance that can't attack,
	...
	private void thinkActive()
	{
		...
		// Add allTemplates autoAttackable L2Character in L2Attackable Aggro Range to its _aggroList with 0 damage and 1 hate
		// A L2Attackable isn't aggressive during 10s after its spawn because _globalAggro is set to -10
		if (_globalAggro >= 0)
		{
			// Get allTemplates visible objects inside its Aggro Range
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Update the attack timeout if actor is running</li> <li>If target is dead or timeout is expired, stop this attack and set the Intention to AI_INTENTION_ACTIVE</li> <li>Call allTemplates L2Object of its Faction inside the Faction Range</li> <li>Chose a target and order to attack it with magic skill
	...
	private void thinkAttack(){
		...
		{
			// Call allTemplates L2Object of its Faction inside the Faction Range
			if (((L2NpcInstance) actor).getFactionId() != null)
			{
				String faction_id = ((L2NpcInstance) actor).getFactionId();
				
				// Go through allTemplates L2Object that belong to its faction
				for (L2Object obj : actor.getKnownList().getKnownObjects().values())
				{
			...
			// Get allTemplates information needed to chose between physical or magical attack
			L2Skill[] skills = null;
			double dist2 = 0;
			int range = 0;
			...
			try
			{
				...
				range = actor.getPhysicalAttackRange() + (int) actor.getCollisionRadius() + (int) getAttackTarget().getCollisionRadius();
			}
