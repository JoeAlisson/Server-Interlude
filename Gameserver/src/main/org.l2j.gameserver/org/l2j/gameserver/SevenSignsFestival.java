...
import org.l2j.gameserver.model.actor.instance.L2FestivalMonsterInstance;
...
import org.l2j.gameserver.model.entity.database.SevenSignsStatus;
...
public class SevenSignsFestival implements SpawnListener {
    ...
    // ////////////////////// \\\\\\\\\\\\\\\\\\\\\\\\\\
    /*
     * The following contains allTemplates the necessary spawn data for: - Player Start Locations - Witches - Monsters - Chests All data is given by: X, Y, Z (coords), Heading, NPC ID (if necessary) This may be moved externally in time, but the data should not change.
     */
     ...
     * If updateSettings = true, then allTemplates Seven Signs data is updated in the database.
     *
    ...
    /**
     * Used to reset allTemplates festival data at the beginning of a new quest event period.
     *
     * @param updateSettings
     */
    ...
    /**
     * Used to send a "shout" message to allTemplates players currently present in an Oracle. Primarily used for Festival Guide and Witch related speech.
     *
     * @param senderName
     * @param message
     */
    ...
    /**
     * The FestivalManager class is the main runner of allTemplates the festivals. It is used for easier integration and management of allTemplates running festivals.
     *
     * @author Tempy
     */
    private class FestivalManager implements Runnable {
        ...
        public synchronized void run() {
            ...
            /* PROPOGATION */
            // Sequentially set allTemplates festivals to begin, spawn the Festival Witch and notify participants.
            for (L2DarknessFestival festivalInst : _festivalInstances.values()) {
            ...
            // After a short time period, move allTemplates idle spawns to the center of the arena.
            try {
            ...
            // After another short time period, again move allTemplates idle spawns to the center of the arena.
            try {
            ...
            /* TERMINATION */
            // Sequentially begin the ending sequence for allTemplates running festivals.
            for (L2DarknessFestival festivalInst : _festivalInstances.values()) {
    ...
    /**
     * Each running festival is represented by an L2DarknessFestival class. It contains allTemplates the spawn information and data for the running festival. All festivals are managed by the FestivalManager class, which must be initialized first.
     *
     * @author Tempy
     */
    private class L2DarknessFestival {
        ...
        protected void festivalInit() {
            ...
            // Teleport allTemplates players to arena and notify them.
            if (_participants.size() > 0) {
                try {
                    for (L2PcInstance participant : _participants) {
                        ...
                        // Remove allTemplates buffs from allTemplates participants on entry. Works like the skill Cancel.
            ...
            // Send a message to allTemplates participants from the witch.
        ...
        protected void unspawnMobs() {
            // Delete allTemplates the NPCs in the current festival arena.
            if (_witchInst != null) {
                _witchInst.deleteMe();
            }
